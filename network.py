            ...
            try:
                ...
                if msg["type"] == "LIST":
                    ...
                    self.messages.append(
                        {"text": msg["msg"], "time": pygame.time.get_ticks()}
                    )
...
def lobby_screen(screen, net, font, title_font):
    ...
    while net.app_state == "LOBBY":
        ...
        pygame.draw.rect(
            screen,
            Theme.BTN_HOVER if active_room else Theme.BTN,
            room_rect,
            border_radius=10,
        )
        pygame.draw.rect(
            screen,
            Theme.BTN_HOVER if active_nick else Theme.BTN,
            nick_rect,
            border_radius=10,
        )

        screen.blit(
            font.render(f"Room: {room_text}", True, Theme.TEXT),
            (room_rect.x + 10, room_rect.y + 10),
        )
        screen.blit(
            font.render(f"Nick: {nick_text}", True, Theme.TEXT),
            (nick_rect.x + 10, nick_rect.y + 10),
        )
        ...
        for name, count in net.rooms.items():
            ...
            screen.blit(
                font.render(f"{name} ({count}/2)", True, Theme.TEXT),
                (r.x + 10, r.y + 10),
            )
            y += 55
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for name in net.rooms:
                    if pygame.Rect(WIDTH // 2 - 200, y, 400, 45).collidepoint(
                        event.pos
                    ):
                        net.nickname = nick_text
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if active_room:
                    ...
                    elif len(room_text) < 12:
                        room_text += event.unicode
                if active_nick:
                    if event.key == pygame.K_BACKSPACE:
                        nick_text = nick_text[:-1]
                    elif len(nick_text) < 12:
                        nick_text += event.unicode
...
def waiting_screen(screen, net, font, leave_btn):
    ...
    mouse = pygame.mouse.get_pos()
    screen.blit(
        font.render("Waiting for opponent...", True, Theme.TEXT),
    ...
    while net.app_state == "GAME":
        ...
        if net.game_state:
            ...
            if typing:
                ...
                screen.blit(
                    font.render(f"Say: {chat_input}{cursor}", True, Theme.TEXT),
                    (15, HEIGHT - 45),
                )
            else:
            ...
            if (
                not net.game_state.game_over
                and net.game_state.turn == net.player_id
                and not typing
                and not confirm_leave
                and not net.opponent_disconnected
            ):
        ...
        if local_game_over and net.game_state:
            ...
            draw_overlay(
                ...
                title_font,
                winner_color,
                board=net.game_state.board,
                winner=net.game_state.winner,
            )
