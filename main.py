...
def difficulty_menu_screen(screen, font, title_font):
    ...
    while True:
        ...
        screen.blit(
            title_font.render("CHOOSE DIFFICULTY", True, Theme.TEXT),
            (WIDTH // 2 - 220, 100),
        )
...
def play_local_game(screen, font, title_font, is_ai=False, ai_depth=3):
    ...
    while True:
        ...
        if not game_over and is_ai and turn == 2:
            col, _ = Connect4AI.minimax(
                board, ai_depth, -float("inf"), float("inf"), True
            )
        ...
        if not game_over:
            ...
            UI.draw_text_with_outline(
                screen, f"PLAYER {turn}'S TURN", font, color, (WIDTH // 2, 30)
            )

            col = mouse[0] // CELL_SIZE
            draw_token(
                screen, turn, int(col * CELL_SIZE + CELL_SIZE / 2), TOP_MARGIN - 50
            )
...
def main():
    ...
    while state != "QUIT":
        ...
        if state == "MAIN_MENU":
            ...
            state = play_local_game(
                screen, font, title_font, is_ai=True, ai_depth=ai_depth
            )
