...
class Connect4AI:
    ...
    def get_random_move(board):
        valid_locations = [c for c in range(GameLogic.COLS) if GameLogic.is_valid_location(board, c)]
    ...
    def minimax(board, depth, alpha, beta, maximizingPlayer):
        valid_locations = [c for c in range(GameLogic.COLS) if GameLogic.is_valid_location(board, c)]
        is_terminal = GameLogic.check_win(board, 1) or GameLogic.check_win(board, 2) or len(valid_locations) == 0
