...
class Assets:
    ...
    def load():
        ...
        Assets.TOKEN_P1 = pygame.image.load(
            os.path.join("assets", "token_p1.png")
        ).convert_alpha()
        Assets.TOKEN_P1 = pygame.transform.smoothscale(Assets.TOKEN_P1, token_size)

        Assets.TOKEN_P2 = pygame.image.load(
            os.path.join("assets", "token_p2.png")
        ).convert_alpha()
        Assets.TOKEN_P2 = pygame.transform.smoothscale(Assets.TOKEN_P2, token_size)

        Assets.BOARD = pygame.image.load(
            os.path.join("assets", "board.png")
        ).convert_alpha()
        Assets.BOARD = pygame.transform.smoothscale(
            Assets.BOARD, (COLS * CELL_SIZE, ROWS * CELL_SIZE)
        )
...
class Button:
    ...
    def draw(self, screen, mouse):
        ...
        pygame.draw.rect(
            screen,
            self.highlight_color if is_hovered else self.hover_color,
            highlight_rect,
            border_radius=12,
        )
    ...
    def clicked(self, event, mouse):
        ...
        return event.type == pygame.MOUSEBUTTONDOWN and current_top_rect.collidepoint(
            mouse
        )
...
def draw_overlay(screen, text, font, color, board=None, winner=None):
    ...
    if board and winner and winner in [1, 2]:
        ...
        for r, c in winning_coords:
            ...
            pygame.draw.circle(
                glow_surf, (255, 215, 0, 150), (glow_radius, glow_radius), glow_radius
            )
...
def draw_chat_history(screen, net, font):
    ...
    visible_messages = (
        net.messages[-max_messages:]
        if len(net.messages) > max_messages
        else net.messages
    )
